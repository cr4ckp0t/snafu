								...
								if (ticket.field === 'state') {
									...
									if (ticketType !== 'absolute_install') {
										...
										if (ticketType === 'rhs_build' || ticketType === 'rhs_reclaim' || ticketType === 'rhs_restock' || ticketType === 'rhs_repair' || ticketType === 'rhs_decommission') {
											...
											if (rootCause.sys_id !== brokenHost.sys_id) {
											...
											if (rootCause.sys_id !== replaceHost.sys_id) {
												snafuSetDisplayValue('cmdb_ci', replaceHost.sys_id, replaceHost.name);
												snafuFlash('cmdb_ci');
											}
										} else {
											// desktop services value is 5a8d6816a1cf38003a42245d1035d56e
											if (rootCause.sys_id !== '5a8d6816a1cf38003a42245d1035d56e') {
...
function snafuReplaceWildcards(strIn) {
	...
	const wildcards = {
		// global
		"{ASSIGN_GROUP}": "(snafuIsVarEmpty(g_form.getReference('assignment_group').name) === false) ? g_form.getReference('assignment_group').name : 'UNKNOWN';",
		...
		"{ROOT_CAUSE}": "(snafuIsVarEmpty(g_form.getReference('cmdb_ci').name) === false) ? g_form.getReference('cmdb_ci').name : 'UNKNOWN';",
		"{TECH_NAME}": "(snafuIsVarEmpty(g_form.getReference('assigned_to').name) === false) ? snafuUcwords(g_form.getReference('assigned_to').name) : 'UNKNOWN';",
		"{TICKET}": "(snafuIsVarEmpty(g_form.getValue('number')) === false) ? g_form.getValue('number') : 'UNKNOWN';",
		
		//incident only
		"{INC_ADDR}": "(snafuIsVarEmpty(g_form.getReference('u_street_address').u_name) === false) ? snafuUcwords(g_form.getReference('u_street_address').u_name) : 'UNKNOWN';",
		...
		"{INC_CUSTOMER}": "(snafuIsVarEmpty(g_form.getReference('caller_id').name) === false) ? snafuUcwords(g_form.getReference('caller_id').name) : 'UNKNOWN';",
		"{INC_CUST_FNAME}": "(snafuIsVarEmpty(g_form.getReference('caller_id').first_name) === false) ? snafuUcwords(g_form.getReference('caller_id').first_name) : 'UNKNOWN';",
		"{INC_CUST_LNAME}": "(snafuIsVarEmpty(g_form.getReference('caller_id').last_name) === false) ? snafuUcwords(g_form.getReference('caller_id').last_name) : 'UNKNOWN';",
		"{INC_DETAIL_DESC}": "(snafuIsVarEmpty(g_form.getValue('description')) === false) ? g_form.getValue('description') : 'UNKNOWN';",
		"{INC_EMAIL}": "(snafuIsVarEmpty(g_form.getReference('caller_id').email) === false) ? g_form.getReference('caller_id').email.toLowerCase() : 'UNKNOWN';",
		"{INC_IMPACT}": "(snafuIsVarEmpty(g_form.getValue('impact')) === false) ? g_form.getValue('impact') : 'UNKNOWN';",
		"{INC_KB}": "(snafuIsVarEmpty(g_form.getReference('u_kb_article').number) === false) ? g_form.getReference('u_kb_article').number : 'UNKNOWN';",
		"{INC_LOC_TYPE}": "(snafuIsVarEmpty(g_form.getValue('u_location_type')) === false) ? g_form.getValue('u_location_type') : 'UNKNOWN';",
		"{INC_PRACTICE}": "(snafuIsVarEmpty(g_form.getReference('u_practice_name').name) === false) ? snafuUcwords(g_form.getReference('u_practice_name').name) : 'UNKNOWN';",
		...
		"{REQUESTED_FOR}": "(snafuIsVarEmpty(g_form.getReference('request_item.u_requested_for').name) === false) ? snafuUcwords(g_form.getReference('request_item.u_requested_for').name) : 'UNKNOWN';",
		...
		// hot swap only
		"{BROKEN_ASSET}": "(snafuIsVarEmpty(g_form.getReference('rhs_comp_name').asset_tag) === false) ? g_form.getReference('rhs_comp_name').asset_tag : 'UNKNOWN';",
		"{BROKEN_HOSTNAME}": "(snafuIsVarEmpty(g_form.getReference('rhs_comp_name').name) === false) ? g_form.getReference('rhs_comp_name').name : 'UNKNOWN';",
		"{BROKEN_MODEL}": "(snafuIsVarEmpty(g_form.getReference('rhs_comp_name').model_id) === false) ? snafuGetComputerModel(g_form.getReference('rhs_comp_name').model_id) : 'UNKNOWN';",
		"{BROKEN_SERIAL}": "(snafuIsVarEmpty(g_form.getReference('rhs_comp_name').serial_number) === false) ? g_form.getReference('rhs_comp_name').serial_number : 'UNKNOWN';",
		"{RELATED_INC}": "(snafuIsVarEmpty(g_form.getReference('rhs_inc').number) === false) ? g_form.getReference('rhs_inc').number : 'UNKNOWN';",
		"{REPLACE_ASSET}": "(snafuIsVarEmpty(g_form.getReference('rhs_replacement_computer').asset_tag) === false) ? g_form.getReference('rhs_replacement_computer').asset_tag : 'UNKNOWN';",
		"{REPLACE_BUILD}": "(snafuIsVarEmpty(g_form.getValue('rhs_software')) === false) ? g_form.getValue('rhs_software').replace(\/(?:\\r\\n|\\r|\\n)\/g, ', ') : 'UNKNOWN';",
		"{REPLACE_CUSTOMER}": "(snafuIsVarEmpty(g_form.getReference('rhs_user').name) === false) ? snafuUcwords(g_form.getReference('rhs_user').name) : 'UNKNOWN';",
...
function snafuInfoMessage(msg) {
	if (g_form && snafuIsVarEmpty(g_form) === false) {
    ...
    if (document.getElementById('incident.incident_state') !== null) {
		...
		switch (shortDesc) {
			...
			case 'Smart/Remote Hands Request':
				...
				break;
			default:
				if (shortDesc.indexOf('CANCELLED TASK') !== -1) {
					...
					if (taskName in snafuAutoTickets) {
						...
						var catItem = g_form.getReference('request_item.cat_item').name;
						if (catItem.indexOf('Equipment Move') !== -1) {
		...
		if (labelFields === undefined) {
			...
			for (var field in labelFields) {
				if (field !== 'ticketType') {
					...
					if ((labelType === 'reclaim' || labelType === 'repair') && (field === 'TEXT_5' || field === 'TEXT_1')) {
...
function snafuGetDymoLabelXml(type) {
	...
	switch (type) {
		...
		// reclaim label
		case 'reclaim':
			return '<?xml version="1.0" encoding="utf-8"?><DieCutLabel Version="8.0" Units="twips"><PaperOrientation>Landscape</PaperOrientation><Id>Address</Id><IsOutlined>false</IsOutlined><PaperName>30252 Address</PaperName><DrawCommands><RoundRectangle X="0" Y="0" Width="1581" Height="5040" Rx="270" Ry="270" /></DrawCommands><ObjectInfo><TextObject><Name>TEXT</Name><ForeColor Alpha="255" Red="0" Green="0" Blue="0" /><BackColor Alpha="0" Red="255" Green="255" Blue="255" /><LinkedObjectName /><Rotation>Rotation0</Rotation><IsMirrored>False</IsMirrored><IsVariable>False</IsVariable><GroupID>-1</GroupID><IsOutlined>False</IsOutlined><HorizontalAlignment>Left</HorizontalAlignment><VerticalAlignment>Middle</VerticalAlignment><TextFitMode>None</TextFitMode><UseFullFontHeight>True</UseFullFontHeight><Verticalized>False</Verticalized><StyledText><Element><String xml:space="preserve">&lt;SERIAL&gt;</String><Attributes><Font Family="Arial" Size="9" Bold="False" Italic="False" Underline="False" Strikeout="False" /><ForeColor Alpha="255" Red="0" Green="0" Blue="0" HueScale="100" /></Attributes></Element></StyledText></TextObject><Bounds X="331" Y="1277.82592773438" Width="3115.73413085938" Height="187.199996948242" /></ObjectInfo><ObjectInfo><DateTimeObject><Name>QuarantineDate</Name><ForeColor Alpha="255" Red="0" Green="0" Blue="0" /><BackColor Alpha="0" Red="255" Green="255" Blue="255" /><LinkedObjectName /><Rotation>Rotation0</Rotation><IsMirrored>False</IsMirrored><IsVariable>False</IsVariable><GroupID>-1</GroupID><IsOutlined>False</IsOutlined><HorizontalAlignment>Left</HorizontalAlignment><VerticalAlignment>Middle</VerticalAlignment><TextFitMode>None</TextFitMode><UseFullFontHeight>True</UseFullFontHeight><Verticalized>False</Verticalized><DateTimeFormat>ShortSystemDate</DateTimeFormat><Font Family="Arial" Size="9" Bold="False" Italic="False" Underline="False" Strikeout="False" /><PreText>Date Quarantined: </PreText><PostText /><IncludeTime>True</IncludeTime><Use24HourFormat>False</Use24HourFormat></DateTimeObject><Bounds X="331" Y="648.052490234375" Width="4622" Height="221.363922119141" /></ObjectInfo><ObjectInfo><TextObject><Name>Tech</Name><ForeColor Alpha="255" Red="0" Green="0" Blue="0" /><BackColor Alpha="0" Red="255" Green="255" Blue="255" /><LinkedObjectName /><Rotation>Rotation0</Rotation><IsMirrored>False</IsMirrored><IsVariable>False</IsVariable><GroupID>-1</GroupID><IsOutlined>False</IsOutlined><HorizontalAlignment>Right</HorizontalAlignment><VerticalAlignment>Middle</VerticalAlignment><TextFitMode>None</TextFitMode><UseFullFontHeight>True</UseFullFontHeight><Verticalized>False</Verticalized><StyledText><Element><String xml:space="preserve">Tech: Adam Koch</String><Attributes><Font Family="Arial" Size="9" Bold="False" Italic="False" Underline="False" Strikeout="False" /><ForeColor Alpha="255" Red="0" Green="0" Blue="0" HueScale="100" /></Attributes></Element></StyledText></TextObject><Bounds X="3163.158203125" Y="1277" Width="1789.84191894531" Height="187.199996948242" /></ObjectInfo><ObjectInfo><TextObject><Name>TEXT_4</Name><ForeColor Alpha="255" Red="0" Green="0" Blue="0" /><BackColor Alpha="0" Red="255" Green="255" Blue="255" /><LinkedObjectName /><Rotation>Rotation0</Rotation><IsMirrored>False</IsMirrored><IsVariable>False</IsVariable><GroupID>-1</GroupID><IsOutlined>False</IsOutlined><HorizontalAlignment>Left</HorizontalAlignment><VerticalAlignment>Middle</VerticalAlignment><TextFitMode>None</TextFitMode><UseFullFontHeight>True</UseFullFontHeight><Verticalized>False</Verticalized><StyledText><Element><String xml:space="preserve">Quarantine</String><Attributes><Font Family="Arial" Size="11" Bold="True" Italic="False" Underline="False" Strikeout="False" /><ForeColor Alpha="255" Red="0" Green="0" Blue="0" HueScale="100" /></Attributes></Element></StyledText></TextObject><Bounds X="331" Y="58" Width="4622" Height="242.819625854492" /></ObjectInfo><ObjectInfo><TextObject><Name>TEXT_5</Name><ForeColor Alpha="255" Red="0" Green="0" Blue="0" /><BackColor Alpha="0" Red="255" Green="255" Blue="255" /><LinkedObjectName /><Rotation>Rotation0</Rotation><IsMirrored>False</IsMirrored><IsVariable>False</IsVariable><GroupID>-1</GroupID><IsOutlined>False</IsOutlined><HorizontalAlignment>Left</HorizontalAlignment><VerticalAlignment>Middle</VerticalAlignment><TextFitMode>None</TextFitMode><UseFullFontHeight>True</UseFullFontHeight><Verticalized>False</Verticalized><StyledText><Element><String xml:space="preserve">Reason:</String><Attributes><Font Family="Arial" Size="9" Bold="False" Italic="False" Underline="False" Strikeout="False" /><ForeColor Alpha="255" Red="0" Green="0" Blue="0" HueScale="100" /></Attributes></Element></StyledText></TextObject><Bounds X="331" Y="967.651062011719" Width="677.000000000001" Height="187.199996948242" /></ObjectInfo><ObjectInfo><TextObject><Name>RITM#</Name><ForeColor Alpha="255" Red="0" Green="0" Blue="0" /><BackColor Alpha="0" Red="255" Green="255" Blue="255" /><LinkedObjectName /><Rotation>Rotation0</Rotation><IsMirrored>False</IsMirrored><IsVariable>False</IsVariable><GroupID>-1</GroupID><IsOutlined>False</IsOutlined><HorizontalAlignment>Right</HorizontalAlignment><VerticalAlignment>Middle</VerticalAlignment><TextFitMode>None</TextFitMode><UseFullFontHeight>True</UseFullFontHeight><Verticalized>False</Verticalized><StyledText><Element><String xml:space="preserve">RITMXXXXXXX</String><Attributes><Font Family="Arial" Size="9" Bold="True" Italic="False" Underline="False" Strikeout="False" /><ForeColor Alpha="255" Red="0" Green="0" Blue="0" HueScale="100" /></Attributes></Element></StyledText></TextObject><Bounds X="3326.11669921875" Y="360" Width="1626.88342285156" Height="187.199996948242" /></ObjectInfo><ObjectInfo><BarcodeObject><Name>BARCODE</Name><ForeColor Alpha="255" Red="0" Green="0" Blue="0" /><BackColor Alpha="0" Red="255" Green="255" Blue="255" /><LinkedObjectName>RITM#</LinkedObjectName><Rotation>Rotation0</Rotation><IsMirrored>False</IsMirrored><IsVariable>True</IsVariable><GroupID>-1</GroupID><IsOutlined>False</IsOutlined><Text>RITMXXXXXXX</Text><Type>Code39</Type><Size>Small</Size><TextPosition>None</TextPosition><TextFont Family="Arial" Size="8" Bold="False" Italic="False" Underline="False" Strikeout="False" /><CheckSumFont Family="Arial" Size="8" Bold="False" Italic="False" Underline="False" Strikeout="False" /><TextEmbedding>None</TextEmbedding><ECLevel>0</ECLevel><HorizontalAlignment>Left</HorizontalAlignment><QuietZonesPadding Left="0" Top="0" Right="0" Bottom="0" /></BarcodeObject><Bounds X="331" Y="322.519256591797" Width="3145.45361328125" Height="265.037841796875" /></ObjectInfo><ObjectInfo><TextObject><Name>TEXT_1</Name><ForeColor Alpha="255" Red="0" Green="0" Blue="0" /><BackColor Alpha="0" Red="255" Green="255" Blue="255" /><LinkedObjectName /><Rotation>Rotation0</Rotation><IsMirrored>False</IsMirrored><IsVariable>False</IsVariable><GroupID>-1</GroupID><IsOutlined>False</IsOutlined><HorizontalAlignment>Left</HorizontalAlignment><VerticalAlignment>Top</VerticalAlignment><TextFitMode>ShrinkToFit</TextFitMode><UseFullFontHeight>True</UseFullFontHeight><Verticalized>False</Verticalized><StyledText><Element><String xml:space="preserve">&lt;REASON&gt;</String><Attributes><Font Family="Arial" Size="9" Bold="False" Italic="False" Underline="False" Strikeout="False" /><ForeColor Alpha="255" Red="0" Green="0" Blue="0" HueScale="100" /></Attributes></Element></StyledText></TextObject><Bounds X="1068" Y="975" Width="3885" Height="210" /></ObjectInfo></DieCutLabel>';
			break;
